...
*The population groups at risk that the CRCs are looking for during their EPIC pre-screen include: African-American males between the ages of 15-29, Women of Color (WOC), and individuals who identify within the transgender or non-binary community. If someone belonging to any of these populations groups are ave no evidence of a HIV test within the past year, they are of eligible for CRC approach (i.e. Risk group w/o test).*

The bar chart below shows the total number of encounters where the individual falls within these population groups at risk. The blue labels on the end of each bar in the chart shows the percentage of the total that were screened and eligible for the EIP Service Center during the ED General Screen (n = `r total_eip_eligible`).
...
The lollipop chart below shows the percentages for each risk group determined by the CRC during the EIP EPIC pre-screen. The denominator for these percentages are the total number of encounters selected for the EIP Service Center protocol (n = `r total_eip_eligible`).
