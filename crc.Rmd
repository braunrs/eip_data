...
crc <- read.csv('M:/EIP Data/EIP-CRC Screening/Original Data/EIP CRC Screening Data 20230623_0853.csv')
dim(crc)
head(crc)
...
The following graph shows the daily percentage of the total selected for the EIP Service Center by the CRC on the ED General Screen based on all those screened. The blue line shows the total percentage overall for the time of the EIP Service Center screening.
...
The lollipop chart below shows the percentages for each risk group determined by the CRC Epic pre-screen. The percentages displayed are the percent of the total screened for the EIP Service Center. 
...
crc_risk_table[nrow(crc_risk_table) + 1,] <- list('Risk group w/o test', total_risk_group_not_tested_percent)
...
There are several assessment questions that the CRCs discuss with the individual to evaluate their situation beyond what is evident in Epic.
