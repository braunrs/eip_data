...
# Resource Database
resources <- read.csv('M:/EIP Data/EIP Database/EIP Resource Database/Original Data/EIP Resource Database 20240812_0937.csv')
dim(resources)
...
summary(eip$enc_date)
enc <- eip[which(eip$enc_date >= '2022-12-01' & eip$enc_date <= '2024-12-31'), ]
...
ht_unsafe_work_assessed <- enc[which(enc$unsafe_work == 0 | enc$unsafe_work == 1), ]
...
ht_forced_work_assessed <- enc[which(enc$forced_work == 0 | enc$forced_work == 1), ]
...
ht_fear_work_assessed <- enc[which(enc$fear_leave_work == 0 | enc$fear_leave_work == 1), ]
...
ht_exchange_sex_assessed <- enc[which(enc$exchange_for_sex == 0 | enc$exchange_for_sex == 1), ]
...
enc$ht_exchange_sex_full[enc$exchange_for_sex == 1 |
                           enc$sex_receivemoney___1 == 1 |
                           enc$sex_receivemoney___2 == 1 |
                           enc$sex_receivemoney___3 == 1 |
                           enc$sex_receivemoney___4 == 1 |
...
reactable(resource_summary_tbl,
          theme = reactableTheme(
            color = "hsl(233, 9%, 87%)",
            backgroundColor = "hsl(233, 9%, 19%)",
            borderColor = "hsl(233, 9%, 22%)",
