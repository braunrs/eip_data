...
table(enc$ltc_services___5)
table(enc$additional_services___3)
enc$prep_ltc[enc$ltc_services___5 == 1 | enc$additional_services___3 == 1] <- 1
table(enc$prep_ltc)

table(enc$prep_ltc)
prep_ltc <- enc[which(enc$prep_ltc == 1), ]
dim(prep_ltc)
prep_ltc <- nrow(prep_ltc)
prep_ltc
...
table(enc$ltc_prep_prevented_rsn___16)
enc$prep_already_linked <- NA
enc$prep_already_linked[enc$prep_ltc == 1 &
                         (enc$ltc_prep_declined_rsn___23 == 1 |
                            enc$ltc_prep_prevented_rsn___16 == 1)] <- 1
...
enc$prep_deceased <- NA
enc$prep_deceased[enc$prep_ltc == 1 &
...
enc$prep_incarcerated <- NA
enc$prep_incarcerated[enc$prep_ltc == 1 &
...
enc$prep_linkage_declined <- NA
enc$prep_linkage_declined[(enc$prep_ltc == 1 &
...
enc$prep_ongoing_linkage <- NA
enc$prep_ongoing_linkage[enc$prep_ltc == 1 &
...
prep_ineligible_ltc_percent <- percent(prep_ineligible_ltc/prep_ltc, accuracy = 0.1)
...
enc$prep_eligible_ltc[enc$prep_ltc == 1 &
                       (is.na(enc$prep_already_linked) &
                          is.na(enc$prep_deceased) &
...
prep_eligible_ltc_percent <- percent(prep_eligible_ltc/prep_ltc, accuracy = 0.1)
...
table(enc$prep_in_progress)
enc$prep_ltc_days_open <- difftime(Sys.Date(), enc$enc_date, units="days")
table(enc$prep_ltc_days_open)
enc$prep_in_progress_90days[enc$prep_in_progress == 1 &
                             (enc$prep_ltc_days_open > 90)] <- 1
...
grid +
  ...
  geom_rect(xmin = 0, xmax = 22, ymin = 90, ymax = 100, color = NA,
            fill = NA, size = 0.25) +
  annotate('text', x = 10.5, y = 93, label = prep_ltc, size = 2, color = 'darkgreen') +
