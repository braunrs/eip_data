...
# EIP Database Data
eip <- read.csv('M:/EIP Data/EIP Database/REDCap 2024/EIP Database Data 20240604_0831.csv')
dim(eip)
...
#| fig-cap: "** *Unable to match records means that we are unable to match the EIP follow-up data with the data pulled from the Epic HCV Dashboard. The reason(s) for why these records are unable to be matched are: Tested under different MRN than patient record (n = 8).*"
...
Additional items still in development for this report include: Adding in a visualization for monthly testing/positivity numbers; Add reasons for no follow-up (rsns linkage declined/prevented)
