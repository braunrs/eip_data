...
table(enc$additional_services___2)
enc$housing_ltc[enc$additional_services___2 == 1] <- 1
table(enc$housing_ltc)

table(enc$housing_ltc)
housing_ltc <- enc[which(enc$housing_ltc == 1), ]
dim(housing_ltc)
housing_ltc <- nrow(housing_ltc)
housing_ltc
...
table(enc$ltc_housing_prevented_rsn___16)
enc$housing_already_linked <- NA
enc$housing_already_linked[enc$housing_ltc == 1 &
                         (enc$ltc_housing_declined_rsn___23 == 1 |
                            enc$ltc_housing_prevented_rsn___16 == 1)] <- 1
...
enc$housing_deceased <- NA
enc$housing_deceased[enc$housing_ltc == 1 &
...
enc$housing_incarcerated <- NA
enc$housing_incarcerated[enc$housing_ltc == 1 &
...
enc$housing_linkage_declined <- NA
enc$housing_linkage_declined[(enc$housing_ltc == 1 &
...
enc$housing_ongoing_linkage <- NA
enc$housing_ongoing_linkage[enc$housing_ltc == 1 &
...
housing_ineligible_ltc_percent <- percent(housing_ineligible_ltc/housing_ltc, accuracy = 0.1)
...
enc$housing_eligible_ltc[enc$housing_ltc == 1 &
                       (is.na(enc$housing_already_linked) &
                          is.na(enc$housing_deceased) &
...
housing_eligible_ltc_percent <- percent(housing_eligible_ltc/housing_ltc, accuracy = 0.1)
...
table(enc$housing_in_progress)
enc$housing_ltc_days_open <- difftime(Sys.Date(), enc$enc_date, units="days")
table(enc$housing_ltc_days_open)
enc$housing_in_progress_90days[enc$housing_in_progress == 1 &
                             (enc$housing_ltc_days_open > 90)] <- 1
...
grid +
  ...
  geom_rect(xmin = 0, xmax = 22, ymin = 90, ymax = 100, color = NA,
            fill = NA, size = 0.25) +
  annotate('text', x = 10.5, y = 93, label = housing_ltc, size = 2, color = 'dodgerblue2') +
