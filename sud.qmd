...
nalox <- read.csv('M:/EIP Data/EIP Database/EIP Naloxone Distribution Database/EIP Naloxone Distribution/EIP Naloxone Distribution Data 20240108_1343.csv')
dim(nalox)
...
table(enc$sud_screen_positive, enc$fts_knowledge)
enc$sud_pos_fts_knowledge_assessed[enc$sud_screen_positive == 1 &
                                     !is.na(enc$fts_knowledge)] <- 1
...
table(enc$sud_screen_positive, enc$fts_use)
...
enc$fts_offered_assessed[!is.na(enc$fts_offered)] <- 1
...
table(enc$sud_screen_positive, enc$fts_offered)
enc$sud_pos_fts_interest_assessed[enc$sud_screen_positive == 1 &
                                    !is.na(enc$fts_offered)] <- 1
...
monthly_fts_distribution <- select(enc, enc_date, record_id, fts_provided, fts_number, fts_number_other)
monthly_fts_distribution <- monthly_fts_distribution[which(monthly_fts_distribution$fts_provided == 1), ]
dim(monthly_fts_distribution)
head(monthly_fts_distribution)
monthly_fts_distribution$fts_number_other[monthly_fts_distribution$fts_number == 1] <- 5
table(monthly_fts_distribution$fts_number_other)
monthly_fts_distribution <- select(monthly_fts_distribution, -record_id, -fts_provided, -fts_number)
head(monthly_fts_distribution)
colnames(monthly_fts_distribution)[2] <- 'fts_number'
monthly_fts_distribution$fts_number[is.na(monthly_fts_distribution$fts_number)] <- 0
head(monthly_fts_distribution)
...
monthly_fts_distributed <- aggregate(fts_number ~ as.yearmon(Date),
                                     monthly_fts_distribution,
                               FUN = sum)
monthly_fts_distributed$Date <- as.Date(as.yearmon(monthly_fts_distributed$`as.yearmon(Date)`))
head(monthly_fts_distributed)
monthly_fts_distributed
...
monthly_fts_distributed <- monthly_fts_distributed[order(monthly_fts_distributed$Date), ]
monthly_fts_distribution_plot <- plot_ly(data = monthly_fts_distributed, x = ~Date, y = ~fts_number, type = 'bar', name = 'FTS Distributed',
                                         text = ~fts_number, textposition = 'auto',
                                         marker = list(color = 'darkseagreen1')) %>%
  layout(yaxis = list(title = 'Total FTS Distributed'))
...
Naloxone distribution is documented in two different programs within EIP: (1) direct naloxone distribution of kits provided through grants, and (2) facilitated distribution where the EIP staff prompt medical staff to distribute naloxone from the Omnicell within UCMC and WCH EDs. The two programs will be discussed separately below. Also, the number of kits distributed from the UCMC and WCH Omnicells will be provided with the direct distribution information because those numbers are reported alongside the distribution of our EIP kits.
