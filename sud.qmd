...
nalox <- read.csv('M:/EIP Data/EIP Database/EIP Naloxone Distribution Database/EIP Naloxone Distribution/EIP Naloxone Distribution Data 20241021_0947.csv')
dim(nalox)
...
#####################################  Formatting Naloxone Database Data  ########################################

dim(nalox)
head(nalox)
summary(nalox$date)
nalox$date <- as.Date(nalox$date, format = '%m/%d/%Y')
summary(nalox$date)

nalox <- nalox[which(nalox$date >= '2023-01-01' & nalox$date <= '2024-12-31'), ]
summary(nalox$date)
dim(nalox)
...
nalox_version_individual <- nalox[which(nalox$version == 1), ]
...
nalox_version_transfer <- nalox[which(nalox$version == 2), ]
...
nalox_version_oad <- nalox[which(nalox$version == 3), ]
...
table(nalox$transfer_type)
nalox_version_transfer_partner <- nalox[which(nalox$version == 2 & nalox$transfer_type == 1), ]
...
table(nalox$transfer_type)
nalox_version_transfer_service <- nalox[which(nalox$version == 2 & nalox$transfer_type == 2), ]
...
table(nalox$transfer_type)
nalox_version_transfer_naloxbox <- nalox[which(nalox$version == 2 & nalox$transfer_type == 3), ]
...
table(nalox$agency)
nalox_agency <- nalox[which(nalox$version == 2 | nalox$version == 3), ]
nalox_agency <- select(nalox_agency, agency)
dim(nalox_agency)
nalox_agency <- distinct(nalox_agency)
dim(nalox_agency)
nalox_agency
nalox_agency <- nalox_agency[which(nalox_agency$agency != ''), ]
nalox_agency


# Number of kits distributed

nalox_dist <- select(nalox, date, ucmc_omnicell, wch_omnicell, number_kits)
head(nalox_dist)
...
monthly_nalox_distribution <- select(nalox_dist, date, site, number_kits)
...
monthly_nalox_distribution <- aggregate(number_kits ~ as.yearmon(Date) + site,
...
sud_pos <- enc[which(enc$sud_screen_positive == 1), ]
dim(sud_pos)
sud_pos <- nrow(sud_pos)
sud_pos

sud_pos_percent <- percent(sud_pos/eip_assessments, accuracy = 0.1)
sud_pos_percent
...
table(enc$fts_provided)
eip_fts_distribution <- enc[which(enc$fts_provided == 1), ]
...
eip_fts_distribution$fts_number_other[eip_fts_distribution$fts_number == 1] <- 5
table(eip_fts_distribution$fts_number_other)
eip_fts_distribution <- select(eip_fts_distribution, -fts_number)
head(eip_fts_distribution)
colnames(eip_fts_distribution)[3] <- 'fts_number'
...
eip_fts_distribution$enc_location[eip_fts_distribution$enc_location == 23] <- 'Street/Outreach'
...
table(nalox$fts)
nalox_fts_distribution <- nalox[which(nalox$fts == 1), ]
nalox_fts_distribution <- select(nalox_fts_distribution, date, setting, fts_number)
head(nalox_fts_distribution)
colnames(nalox_fts_distribution)[1] <- 'enc_date'
colnames(nalox_fts_distribution)[2] <- 'enc_location'
head(nalox_fts_distribution)

nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 1] <- 'CAP'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 2] <- 'Court System'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 3] <- 'ED'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 4] <- 'FQHC/non-LHD Clinic'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 5] <- 'Hospital'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 6] <- 'Jail'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 7] <- 'Leave-Behind (EMS/LEO)'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 8] <- 'Local HD'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 9] <- 'Mobile Unit'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 10] <- 'Online Mail Order'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 11] <- 'Pharmacy'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 12] <- 'QRT'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 13] <- 'School/University'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 14] <- 'Street/Outreach'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 15] <- 'SSP'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 16] <- 'Treatment/Recovery'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 17] <- 'Vending/Dispensing Machine'
nalox_fts_distribution$enc_location[nalox_fts_distribution$enc_location == 0] <- 'Other'
head(nalox_fts_distribution)
dim(nalox_fts_distribution)
...
monthly_fts_distribution <- rbind(eip_fts_distribution, nalox_fts_distribution)
...
enc$substance_use[enc$enc_date >= '2024-08-01' &
                    (enc$audit_c1 > 0 |
                    (enc$marijuana_use > 0 & enc$marijuana_use < 4) |
                      (enc$cocaine_use > 0 & enc$cocaine_use < 4) |
                      (enc$rx_amphetamine_use > 0 & enc$rx_amphetamine_use < 4) |
                      (enc$meth_use > 0 & enc$meth_use < 4) |
                      (enc$inhalants_use > 0 & enc$inhalants_use < 4) |
                      (enc$sedatives_use > 0 & enc$sedatives_use < 4) |
                      (enc$ruffies_use > 0 & enc$ruffies_use < 4) |
                      (enc$hallucinogen_use > 0 & enc$hallucinogen_use < 4) |
                      (enc$street_opioid_use > 0 & enc$street_opioid_use < 4) |
                      (enc$rx_opioid_use > 0 & enc$rx_opioid_use < 4) |
                      (enc$injection_drug_use > 0 & enc$injection_drug_use < 4) |
                      (enc$other_drug_use > 0 & enc$other_drug_use < 4))] <- 1
table(enc$substance_use)

table(enc$substance_use)
substance_use <- enc[which(enc$substance_use == 1), ]
dim(substance_use)
substance_use <- nrow(substance_use)
substance_use



enc$sud_linkage_need[enc$substance_use == 1 & enc$ltc_services___3 == 1] <- 1
table(enc$sud_linkage_need)

table(enc$sud_linkage_need)
sud_linkage_need <- enc[which(enc$sud_linkage_need == 1), ]
dim(sud_linkage_need)
sud_linkage_need <- nrow(sud_linkage_need)
sud_linkage_need

sud_linkage_need_percent <- percent(sud_linkage_need/substance_use, accuracy = 0.1)
sud_linkage_need_percent



# Monthly HCPH SUD Data
monthly_hcph_sud <- select(enc, enc_date, substance_use, sud_linkage_need)
monthly_hcph_sud <- monthly_hcph_sud[which(monthly_hcph_sud$enc_date >= '2024-08-01'), ]
monthly_hcph_sud <- monthly_hcph_sud[which(monthly_hcph_sud$substance_use == 1), ]
head(monthly_hcph_sud)
colnames(monthly_hcph_sud)[2] <- 'substance_use'
colnames(monthly_hcph_sud)[3] <- 'sud_linkage_need'
monthly_hcph_sud$substance_use[is.na(monthly_hcph_sud$substance_use)] <- 0
head(monthly_hcph_sud)
monthly_hcph_sud$sud_linkage_need[is.na(monthly_hcph_sud$sud_linkage_need)] <- 0
head(monthly_hcph_sud)

monthly_hcph_sud$Date <- as.Date(monthly_hcph_sud$enc_date, format="%d/%m/%Y")
head(monthly_hcph_sud)

monthly_hcph_sud <- aggregate(cbind(substance_use, sud_linkage_need) ~ as.yearmon(Date),
                              monthly_hcph_sud,
                              FUN = sum)
...
The Hamilton County Public Health (HCPH) substance use disorder (SUD) data agreement was initiated in July 2024 and the agreement was for data collected from August 1, 2024 through July 31, 2025. This agreement is for EIP to provide HCPH with data regarding SUD screening and follow-up at periodic intervals to determine retention in care. The agreement was to provide data for any patient that endorses any type of substance use, the demographic information of these individuals, as well as their treatment need and follow-up for retention in care. This agreement is for patients seen at UC Medical Center only.

Since the data that HCPH requested for follow-up centered around those that were linked to care, it was decided that aggregate data would be collected for all those individuals screened for SUD, those that endorse substance use within the past year, those that are engaged with the EIP Linkage to Care process, and those that are successfully linked to care. Then, record-level data would be provided for those that agree to follow-up questionnaires at 30, 60, and 90 day time points, with the follow-up questions from HCPH documented.

EIP SUD assessments are determined based on the AUDIT-C for alcohol or NIDA-Modified ASSIST for all other substances. Aggregate-level data is presented here for the following: number screened for SUD, number endorsing current substance use (within the past year), number engaged in SUD linkage, and number successfully linked to care. As of the start of the data sharing agreement for this program, there have been a total of xx encounters where an individual was screened for SUD (xx%, xx/xx). The number of encounters where an individual has endorsed current substance use (within the past year) for any substance is xx (xx%, xx/xx). After a conversation with the EIP HPA and/or LTC Coordinator, there was a total of xx encounters where the individual engaged in our linkage process specifically for SUD (xx%, xx/xx). For those that engaged within our SUD LTC process, there were a total of xx encounters where the individual was successfully linked to care for SUD (xx%, xx/xx). A total of xx agreed to the HCPH 30/60/90 day follow-ups after successful linkage (xx%, xx/xx).

30 day follow-up details
60 day follow-up details
90 day follow-up details
...
Patients are included in this program if they endorse any type of substance use within the past year. As of the start of this program, there have been a total of **`r substance_use` encounters where an individual endorsed any type of substance use**. After discussion with our HPA or additional EIP coordinators, there have been a total of **`r sud_linkage_need` linkage follow-up cases initiated** (*`r sud_linkage_need_percent` of all those endorsing any substance use, `r sud_linkage_need`/`r substance_use`*). This information is summarized in the table below.


```{r, echo=FALSE, warning=FALSE}

monthly_hcph_sud_table <- select(monthly_hcph_sud, Date, substance_use, sud_linkage_need)
colnames(monthly_hcph_sud_table)[2] <- 'Endorse Substance Use'
colnames(monthly_hcph_sud_table)[3] <- 'SUD Linkage Need'
...
Any further analysis is depending on final operations decisions still in process.
...
*It is important to note that the number represented here is the number of individual strips, not the number of kits. Standard FTS kits are in packages of 5 strips, and the EIP HPA can select this as an option when documenting the distribution of FTS to individuals. However, they are also able to document non-standard amounts that are distributed in a free text field. Therefore, anytime they select that they distributed the standard kit of 5 strips, this is counted as five individual strips and combined with the free text field of non-standard distribution.*
...
total_distribution <- monthly_nalox_distribution %>%
  group_by(Date) %>%
  summarise(total_kits = sum(number_kits))
...
The flowchart of the SUD Linkage to Care efforts for EIP are shown below.
