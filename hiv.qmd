...
table(enc$ltc_services___1, enc$diagnosis_hiv)
enc$hiv_status[enc$ltc_services___1 == 1 & enc$diagnosis_hiv == 1] <- 'New HIV Diagnosis'
enc$hiv_status[enc$ltc_services___1 == 1 & enc$diagnosis_hiv == 2] <- 'Previous HIV Diagnosis'
enc$hiv_status[enc$ltc_services___1 == 1 & is.na(enc$diagnosis_hiv)] <- 'Unknown HIV Status'
table(enc$hiv_status)



# HRHS CTR Tested (goal is 900)

table(enc$HRHS, enc$ctr_calc)
enc$ctr_hrhs_tested[enc$ctr_calc == 1 & enc$HRHS == 1] <- 1
table(enc$ctr_hrhs_tested)

table(enc$ctr_hrhs_tested)
ctr_hrhs_tested <- enc[which(enc$ctr_hrhs_tested == 1), ]
dim(ctr_hrhs_tested)
...
# HRHS/WOC CTR Tested (goal is 100)

table(enc$HRHS, enc$WOC, enc$ctr_calc)
enc$ctr_hrhs_woc_tested[enc$ctr_calc == 1 & enc$HRHS == 1 & enc$WOC == 1] <- 1
table(enc$ctr_hrhs_woc_tested)

table(enc$ctr_hrhs_woc_tested)
ctr_hrhs_woc_tested <- enc[which(enc$ctr_hrhs_woc_tested == 1), ]
dim(ctr_hrhs_woc_tested)
ctr_hrhs_woc_tested <- nrow(ctr_hrhs_woc_tested)
ctr_hrhs_woc_tested



# HRHS/Youth CTR Tested (goal is 200)

table(enc$HRHS, enc$Youth, enc$ctr_calc)
enc$ctr_hrhs_youth_tested[enc$ctr_calc == 1 & enc$HRHS == 1 & enc$Youth == 1] <- 1
table(enc$ctr_hrhs_youth_tested)

table(enc$ctr_hrhs_youth_tested)
ctr_hrhs_youth_tested <- enc[which(enc$ctr_hrhs_youth_tested == 1), ]
dim(ctr_hrhs_youth_tested)
ctr_hrhs_youth_tested <- nrow(ctr_hrhs_youth_tested)
ctr_hrhs_youth_tested



# HRHS/IDU CTR Tested (goal is 300)

table(enc$HRHS, enc$IDU, enc$ctr_calc)
enc$ctr_hrhs_idu_tested[enc$ctr_calc == 1 & enc$HRHS == 1 & enc$IDU == 1] <- 1
table(enc$ctr_hrhs_idu_tested)

table(enc$ctr_hrhs_idu_tested)
ctr_hrhs_idu_tested <- enc[which(enc$ctr_hrhs_idu_tested == 1), ]
dim(ctr_hrhs_idu_tested)
ctr_hrhs_idu_tested <- nrow(ctr_hrhs_idu_tested)
ctr_hrhs_idu_tested



# MSM CTR Tested (goal is 100)

table(enc$MSM, enc$ctr_calc)
enc$ctr_msm_tested[enc$ctr_calc == 1 & enc$MSM == 1] <- 1
table(enc$ctr_msm_tested)

table(enc$ctr_msm_tested)
ctr_msm_tested <- enc[which(enc$ctr_msm_tested == 1), ]
dim(ctr_msm_tested)
...
# MSM/Youth CTR Tested (goal is 20)

table(enc$MSM, enc$Youth, enc$ctr_calc)
enc$ctr_msm_youth_tested[enc$ctr_calc == 1 & enc$MSM == 1 & enc$Youth == 1] <- 1
table(enc$ctr_msm_youth_tested)

table(enc$ctr_msm_youth_tested)
ctr_msm_youth_tested <- enc[which(enc$ctr_msm_youth_tested == 1), ]
dim(ctr_msm_youth_tested)
ctr_msm_youth_tested <- nrow(ctr_msm_youth_tested)
ctr_msm_youth_tested



# MSM/IDU CTR Tested (goal is 20)

table(enc$MSM, enc$IDU, enc$ctr_calc)
enc$ctr_msm_idu_tested[enc$ctr_calc == 1 & enc$MSM == 1 & enc$IDU == 1] <- 1
table(enc$ctr_msm_idu_tested)

table(enc$ctr_msm_idu_tested)
ctr_msm_idu_tested <- enc[which(enc$ctr_msm_idu_tested == 1), ]
dim(ctr_msm_idu_tested)
ctr_msm_idu_tested <- nrow(ctr_msm_idu_tested)
ctr_msm_idu_tested



# MSM CTR New Positive (goal is 3-5)

table(enc$MSM, enc$ctr_calc, enc$hiv_status)
enc$ctr_msm_new_dx[enc$ctr_calc == 1 & enc$MSM == 1 & enc$hiv_status == 'New HIV Diagnosis'] <- 1
table(enc$ctr_msm_new_dx)

table(enc$ctr_msm_new_dx)
ctr_msm_new_dx <- enc[which(enc$ctr_msm_new_dx == 1), ]
dim(ctr_msm_new_dx)
ctr_msm_new_dx <- nrow(ctr_msm_new_dx)
ctr_msm_new_dx



# HRHS CTR New Positive (goal is 9)

table(enc$HRHS, enc$ctr_calc, enc$hiv_status)
enc$ctr_hrhs_new_dx[enc$ctr_calc == 1 & enc$HRHS == 1 & enc$hiv_status == 'New HIV Diagnosis'] <- 1
table(enc$ctr_hrhs_new_dx)

table(enc$ctr_hrhs_new_dx)
ctr_hrhs_new_dx <- enc[which(enc$ctr_hrhs_new_dx == 1), ]
dim(ctr_hrhs_new_dx)
ctr_hrhs_new_dx <- nrow(ctr_hrhs_new_dx)
ctr_hrhs_new_dx
...
# HRHS Tested (goal is 875)

table(enc$HRHS, enc$ehe_calc)
enc$ehe_hrhs_tested[enc$ehe_calc == 1 & enc$HRHS == 1] <- 1
table(enc$ehe_hrhs_tested)

table(enc$ehe_hrhs_tested)
ehe_hrhs_tested <- enc[which(enc$ehe_hrhs_tested == 1), ]
dim(ehe_hrhs_tested)
ehe_hrhs_tested <- nrow(ehe_hrhs_tested)
ehe_hrhs_tested



# MSM Tested (goal is 75)

table(enc$MSM, enc$ehe_calc)
enc$ehe_msm_tested[enc$ehe_calc == 1 & enc$MSM == 1] <- 1
table(enc$ehe_msm_tested)

table(enc$ehe_msm_tested)
ehe_msm_tested <- enc[which(enc$ehe_msm_tested == 1), ]
dim(ehe_msm_tested)
...
# MSM/Youth Tested (goal is 40)

table(enc$MSM, enc$Youth, enc$ehe_calc)
enc$ehe_msm_youth_tested[enc$ehe_calc == 1 & enc$MSM == 1 & enc$Youth == 1] <- 1
table(enc$ehe_msm_youth_tested)

table(enc$ehe_msm_youth_tested)
ehe_msm_youth_tested <- enc[which(enc$ehe_msm_youth_tested == 1), ]
dim(ehe_msm_youth_tested)
ehe_msm_youth_tested <- nrow(ehe_msm_youth_tested)
ehe_msm_youth_tested



# MSM/IDU Tested (goal is 12)

table(enc$MSM, enc$IDU, enc$ehe_calc)
enc$ehe_msm_idu_tested[enc$ehe_calc == 1 & enc$MSM == 1 & enc$IDU == 1] <- 1
table(enc$ehe_msm_idu_tested)

table(enc$ehe_msm_idu_tested)
ehe_msm_idu_tested <- enc[which(enc$ehe_msm_idu_tested == 1), ]
dim(ehe_msm_idu_tested)
...
enc$plwha[(enc$history_hiv_test_result == 1 |
             enc$reason_for_offer_category_hiv_test == 7 |
             enc$reason_for_offer_category_hiv_test == 8 |
             enc$offer_outcome_declined_hiv___21 == 1 |
             enc$offer_outcome_declined_hiv_2___21 == 1 |
             enc$offer_outcome_declined_hiv_3___21 == 1 |
             enc$final_test_status_hiv == 1 |
             enc$diagnosis_hiv == 1 |
...
table(enc$ltc_services___1)
enc$hiv_ltc[enc$ltc_services___1 == 1] <- 1
table(enc$hiv_ltc)

table(enc$hiv_ltc)
hiv_ltc <- enc[which(enc$hiv_ltc == 1), ]
dim(hiv_ltc)
...
enc$hiv_already_linked[enc$hiv_ltc == 1 &
...
enc$hiv_deceased[enc$hiv_ltc == 1 &
...
enc$hiv_incarcerated[enc$hiv_ltc == 1 &
...
enc$hiv_linkage_declined[(enc$hiv_ltc == 1 &
...
enc$hiv_ongoing_linkage[enc$hiv_ltc == 1 &
...
enc$hiv_eligible_ltc[enc$hiv_ltc == 1 &
                       (is.na(enc$hiv_already_linked) &
                          is.na(enc$hiv_deceased) &
...
table(enc$hiv_eligible_ltc, enc$hiv_status)
enc$hiv_new_eligible_ltc[enc$hiv_eligible_ltc == 1 &
                           enc$hiv_status == 'New HIV Diagnosis'] <- 1
...
enc$hiv_prev_eligible_ltc[enc$hiv_eligible_ltc == 1 &
                            enc$hiv_status == 'Previous HIV Diagnosis'] <- 1
...
enc$hiv_unk_eligible_ltc[(enc$hiv_eligible_ltc == 1 &
                            (enc$hiv_status == 'Unknown HIV Status' |
                               is.na(enc$hiv_status)))] <- 1
...
table(enc$hiv_in_progress, enc$hiv_status)
...
table(enc$hiv_in_progress)
enc$hiv_ltc_days_open <- difftime(Sys.Date(), enc$enc_date, units="days")
table(enc$hiv_ltc_days_open)
enc$hiv_in_progress_90days[enc$hiv_in_progress == 1 &
                             (enc$hiv_ltc_days_open > 90)] <- 1
...
table(enc$hiv_status, enc$hiv_eligible_ltc)
table(enc$hiv_linked, enc$hiv_status)
...
table(enc$hiv_not_linked, enc$hiv_status)
...
daily_hiv_testing_plot <- ggplot(daily_hiv_tests, aes(x = Date)) +
  geom_line(aes(y = ctr_tests), color = "lightskyblue") +
...
ctr_hrhs_tested_gauge <- plot_ly(
  domain = list(x = c(0, 0.45), y = c(0, 1)),
  value = ctr_hrhs_tested,
  title = list(text = "HRHS CTR Tested", font = list(size = 18)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
    axis =list(range = list(0, 900)),
    bar = list(color = "lightskyblue"),
    bgcolor = "white",
    borderwidth = 2,
    bordercolor = "gray")) 
ctr_hrhs_tested_gauge <- ctr_hrhs_tested_gauge %>%
...
ctr_hrhs_woc_tested_gauge <- plot_ly(
  ...
  title = list(text = "HRHS/WOC CTR Tested", font = list(size = 18)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
...
ctr_hrhs_youth_tested_gauge <- plot_ly(
  ...
  title = list(text = "HRHS/Youth CTR Tested", font = list(size = 18)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
...
ctr_hrhs_idu_tested_gauge <- plot_ly(
  ...
  title = list(text = "HRHS/IDU CTR Tested", font = list(size = 18)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
...
ctr_msm_tested_gauge <- plot_ly(
  ...
  title = list(text = "MSM CTR Tested", font = list(size = 18)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
    axis =list(range = list(0, 100)),
...
ctr_msm_youth_tested_gauge <- plot_ly(
  ...
  title = list(text = "MSM/Youth CTR Tested", font = list(size = 18)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
...
ctr_msm_idu_tested_gauge <- plot_ly(
  ...
  title = list(text = "MSM/IDU CTR Tested", font = list(size = 18)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
    axis =list(range = list(0, 20)),
...
The deliverables for the CTR testing program are shown in the gauge charts below. All deliverables are based on the program funding year, and are counted between January 1st and December 31st to determine whether we meet these goals for the program.


```{r, echo = FALSE}

hiv_gauge_subplot1 <- subplot(ctr_hrhs_tested_gauge, ctr_hrhs_woc_tested_gauge)
hiv_gauge_subplot1

hiv_gauge_subplot2 <- subplot(ctr_hrhs_youth_tested_gauge, ctr_hrhs_idu_tested_gauge)
hiv_gauge_subplot2

hiv_gauge_subplot3 <- subplot(ctr_msm_tested_gauge, ctr_msm_youth_tested_gauge)
hiv_gauge_subplot3

hiv_gauge_subplot4 <- subplot(ctr_msm_idu_tested_gauge)
hiv_gauge_subplot4

hiv_gauge_subplot5 <- subplot(ctr_msm_new_dx_gauge, ctr_hrhs_new_dx_gauge)
hiv_gauge_subplot5
...
ctr_deliverables_tbl <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(ctr_deliverables_tbl) <- c('CTR Deliverable', 'Goal', 'Achieved')
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('HRHS tested', 900, ctr_hrhs_tested)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('HRHS WOC tested', 100, ctr_hrhs_woc_tested)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('HRHS Youth tested', 200, ctr_hrhs_youth_tested)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('HRHS IDU tested', 300, ctr_hrhs_idu_tested)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('MSM tested', 100, ctr_msm_tested)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('MSM Youth tested', 20, ctr_msm_youth_tested)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('MSM IDU tested', 20, ctr_msm_idu_tested)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('MSM New CTR positive', '3-5', ctr_msm_new_dx)
ctr_deliverables_tbl[nrow(ctr_deliverables_tbl) + 1,] <- list('HRHS New CTR positive', 9, ctr_hrhs_new_dx)
reactable(ctr_deliverables_tbl,
          theme = reactableTheme(
            color = "hsl(233, 9%, 87%)",
            backgroundColor = "hsl(233, 9%, 19%)",
            borderColor = "hsl(233, 9%, 22%)",
...
The CTR program also has site and population-specific deliverables that need to be met.


-- At UCMC: 75 MSM tested, 550 HRHS tested
-- At WCH: 10 MSM tested, 100 HRHS tested
-- At Exchange: 10 MSM tested, 150 HRHS tested
-- At Talbert: 5 MSM tested, 100 HRHS tested
...
ehe_hrhs_tested_gauge <- plot_ly(
  domain = list(x = c(0, 1), y = c(0, 1)),
  value = ehe_hrhs_tested,
  title = list(text = "HRHS EHE Tested", font = list(size = 24)),
  type = "indicator",
  mode = "gauge+number",
  gauge = list(
    axis =list(range = list(0, 875)),
    bar = list(color = "darkgreen"),
    bgcolor = "white",
    borderwidth = 2,
    bordercolor = "gray")) 
ehe_hrhs_tested_gauge <- ehe_hrhs_tested_gauge %>%
  layout(margin = list(l=100,r=150))

ehe_hrhs_tested_gauge
...
ehe_deliverables_tbl <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(ehe_deliverables_tbl) <- c('EHE Deliverable', 'Goal', 'Achieved')
ehe_deliverables_tbl[nrow(ehe_deliverables_tbl) + 1,] <- list('HRHS tested', 875, ehe_hrhs_tested)
ehe_deliverables_tbl[nrow(ehe_deliverables_tbl) + 1,] <- list('MSM tested', 75, ehe_msm_tested)
ehe_deliverables_tbl[nrow(ehe_deliverables_tbl) + 1,] <- list('MSM Youth tested', 40, ehe_msm_youth_tested)
ehe_deliverables_tbl[nrow(ehe_deliverables_tbl) + 1,] <- list('MSM IDU tested', 12, ehe_msm_idu_tested)
reactable(ehe_deliverables_tbl,
          theme = reactableTheme(
            color = "hsl(233, 9%, 87%)",
            backgroundColor = "hsl(233, 9%, 19%)",
            borderColor = "hsl(233, 9%, 22%)",
...
Any individual that is identified as a person living with HIV/AIDS (PLWHA), whether newly diagnosed or a previously known positive individual, is meant to have a linkage case started to determine the need and level of follow-up by the EIP Linkage team. All encounters where an individual is identified as a PLWHA is counted, and then their need for linkage is assessed by the linkage team so that the true number of those in need of our services can be determined. The total number of encounters where an individual is identified as a PLWHA is **`r plwha`**, and the total number of linkage cases initiated (*should be equal*) is **`r hiv_ltc`** (*`r hiv_ltc_percent` of those encounters with PLWHA, `r hiv_ltc`/`r plwha`*). A consort diagram of these HIV Linkage cases is shown later.

Once an linkage case is started, the linkage team determines whether the linkage case can be realistically worked and the efforts of our linkage staff can continue follow-up with their outreach capacity. There are several factors that have been determined that make a linkage case "ineleigible" to be worked, and are thus excluded from the total number reported to funding entities when reporting linkage data. These situations are not necessarily mutually exclusive.

If an individual states that they are already in care, or if there is clear evidence within the medical record that the individual is successfully engaging in care, then they are termed "ineligible" due to alrady being linked to care. The total number of HIV linkage cases where the individual was known to already be in care are **`r hiv_already_linked`** (*`r hiv_already_linked_percent` of all HIV linkage cases initiated, `r hiv_already_linked`/`r hiv_ltc`*).

If the individual passes away during their encounter in the emergency department, or while our linkage team is attempting to get them linked to care, then they are termed "ineligible" and are not able to be linked, and thus are excluded. The total number of HIV linkage cases where the individual was deceased before linkage could be completed was **`r hiv_deceased`** (*`r hiv_deceased_percent` of all HIV linkage cases initiated, `r hiv_deceased`/`r hiv_ltc`*).

If the individual is in police custody, or incarcerated while the linkage team is attempting to get them linked to care, then there are often difficulties in establishing communication with the individual and getting them to an appointment with a specialist. While not impossible to link them to care in these situations, it does make it more difficult and could provide significant barriers, and thus are termed "ineligible" if this is the case in that situation. The total number of HIV linkage cases where the individual was incarcerated or in police custody, therefore complicating linkage access, was **`r hiv_incarcerated`** (*`r hiv_incarcerated_percent` of all HIV linkage cases initiated, `r hiv_incarcerated`/`r hiv_ltc`*).

If the individual declines our team's linkage services for any reason, even after multiple offers for support, then they are termed "ineligible" due to not wanting linkage servies at that time. The total number of HIV linkage cases where the individual declined linkage services was **`r hiv_linkage_declined`** (*`r hiv_linkage_declined_percent` of all HIV linkage cases initiated, `r hiv_linkage_declined`/`r hiv_ltc`*).

Due to the length of time that some linkage cases may take, there are several instances where the individual may return and be identified by EIP while they already have ongoing linkage cases. Rather than duplicating the need for linkage, and only maintaining one linkage case at a time, the individual is still marked as identified for linkage, but termed "ineligible" due to ongoing linkage already in progress.The total number of HIV linkage cases where the individual already had an ongoing linkage case in progress was **`r hiv_ongoing_linkage`** (*`r hiv_ongoing_linkage_percent` of all HIV linkage cases initiated, `r hiv_ongoing_linkage`/`r hiv_ltc`*).

There are a total number of **`r hiv_ineligible_ltc`** HIV linkage cases where the individual meets at least one of these "ineligible" criteria (*`r hiv_ineligible_ltc_percent` of all HIV linkage cases initiated, `r hiv_ineligible_ltc`/`r hiv_ltc`*), leaving **`r hiv_eligible_ltc`** HIV linkage cases that are eligible to be worked by our linkage team (*`r hiv_eligible_ltc_percent` of all HIV linkage cases initiated, `r hiv_eligible_ltc`/`r hiv_ltc`*). This is the total number of HIV Linkage cases that is used when reporting our linkage efforts for HIV. Additionally, HIV linkage is broken out by linkage efforts for those who are newly diagnosed and those those are previous positives requiring re-engagement in care. There are **`r hiv_new_eligible_ltc`** linkage cases eligible for individuals who are New Diagnoses (*`r hiv_new_eligible_ltc_percent` of all eligible HIV linkage cases, `r hiv_new_eligible_ltc`/`r hiv_eligible_ltc`*) and there are **`r hiv_prev_eligible_ltc`** linkage cases for individuals who are Previous Diagnoses (*`r hiv_prev_eligible_ltc_percent` of all eligible HIV linkage cases, `r hiv_prev_eligible_ltc`/`r hiv_eligible_ltc`*). It may take some time before it is known whether an individual has a previous diagnosis, or if that instance of when staff identify them as positive is a new diagnosis, depending on the situation. Therefore, it is also tracked on the number of eligible HIV linkage cases where the individual has an "unknown diagnosis" status. This does not mean their status is unknown (*since HIV linkage is only done for those who are positive*), it is unknown whether they are new or previous at the time of the data pull. There are **`r hiv_unk_eligible_ltc`** HIV linkage cases where they are in this unknown state (*`r hiv_unk_eligible_ltc_percent` of all eligible HIV linkage cases, `r hiv_unk_eligible_ltc`/`r hiv_eligible_ltc`*).
...
In addition to a particular percentage that needs to be met for the CTR linkage deliverables, there are also site-specific and population-specific deliverables that are required to meet throughout the funding year. Those deliverables are described and presented below.



- Site deliverables:
-- 75 PLWHA linked at UCMC
-- 15 PLWHA linked at WCH
-- 5 PLWHA linked at Exchange
-- 5 PLWHA linked at Talbert


- Population deliverables:
-- 20 PLWHA/WOC linked
-- 25 PLWHA/Youth linked
-- 25 PLWHA/IDU linked
