...
table(eip$final_test_status_hiv)
eip$hiv_positive_test[eip$final_test_status_hiv == 1] <- 1
table(eip$hiv_positive_test)
hiv_positive_tests <- eip[which(eip$hiv_positive_test == 1), ]
dim(hiv_positive_tests)
hiv_positive_tests <- nrow(hiv_positive_tests)
hiv_positive_tests


# Monthly False Positives
monthly_hiv_pos <- select(eip, enc_date, record_id, hiv_positive_test)
monthly_hiv_pos <- monthly_hiv_pos[which(monthly_hiv_pos$hiv_positive_test == 1), ]
head(monthly_hiv_pos)
monthly_hiv_pos <- select(monthly_hiv_pos, -record_id)
head(monthly_hiv_pos)
monthly_hiv_pos$hiv_positive_test[is.na(monthly_hiv_pos$hiv_positive_test)] <- 0
head(monthly_hiv_pos)

monthly_hiv_pos$Date <- format(as.Date(monthly_hiv_pos$enc_date, format="%d/%m/%Y"),"%m/%Y")
head(monthly_hiv_pos)


monthly_hiv_pos <- aggregate(hiv_positive_test ~ Date,
                             monthly_hiv_pos,
                             FUN = sum)
head(monthly_hiv_pos)
monthly_hiv_pos
...
# Annual False Positives
annual_hiv_pos <- select(eip, enc_date, record_id, hiv_positive_test)
annual_hiv_pos <- annual_hiv_pos[which(annual_hiv_pos$hiv_positive_test == 1), ]
head(annual_hiv_pos)
annual_hiv_pos <- select(annual_hiv_pos, -record_id)
head(annual_hiv_pos)
annual_hiv_pos$hiv_positive_test[is.na(annual_hiv_pos$hiv_positive_test)] <- 0
head(annual_hiv_pos)

annual_hiv_pos$Date <- format(as.Date(annual_hiv_pos$enc_date, format="%d/%m/%Y"),"%Y")
head(annual_hiv_pos)


annual_hiv_pos <- aggregate(hiv_positive_test ~ Date,
                            annual_hiv_pos,
                            FUN = sum)
head(annual_hiv_pos)
annual_hiv_pos
...
table(eip$hiv_false_positive_calc)
hiv_false_positives <- eip[which(eip$hiv_false_positive_calc == 1), ]
dim(hiv_false_positives)
hiv_false_positives <- nrow(hiv_false_positives)
hiv_false_positives


# Monthly False Positives
monthly_hiv_false_pos <- select(eip, enc_date, record_id, hiv_false_positive_calc)
monthly_hiv_false_pos <- monthly_hiv_false_pos[which(monthly_hiv_false_pos$hiv_false_positive_calc == 1), ]
head(monthly_hiv_false_pos)
monthly_hiv_false_pos <- select(monthly_hiv_false_pos, -record_id)
head(monthly_hiv_false_pos)
monthly_hiv_false_pos$hiv_false_positive_calc[is.na(monthly_hiv_false_pos$hiv_false_positive_calc)] <- 0
head(monthly_hiv_false_pos)

monthly_hiv_false_pos$Date <- format(as.Date(monthly_hiv_false_pos$enc_date, format="%d/%m/%Y"),"%m/%Y")
head(monthly_hiv_false_pos)


monthly_hiv_false_pos <- aggregate(hiv_false_positive_calc ~ Date,
                                   monthly_hiv_false_pos,
                                   FUN = sum)
head(monthly_hiv_false_pos)
monthly_hiv_false_pos
...
# Annual False Positives
annual_hiv_false_pos <- select(eip, enc_date, record_id, hiv_false_positive_calc)
annual_hiv_false_pos <- annual_hiv_false_pos[which(annual_hiv_false_pos$hiv_false_positive_calc == 1), ]
head(annual_hiv_false_pos)
annual_hiv_false_pos <- select(annual_hiv_false_pos, -record_id)
head(annual_hiv_false_pos)
annual_hiv_false_pos$hiv_false_positive_calc[is.na(annual_hiv_false_pos$hiv_false_positive_calc)] <- 0
head(annual_hiv_false_pos)

annual_hiv_false_pos$Date <- format(as.Date(annual_hiv_false_pos$enc_date, format="%d/%m/%Y"),"%Y")
head(annual_hiv_false_pos)


annual_hiv_false_pos <- aggregate(hiv_false_positive_calc ~ Date,
                                  annual_hiv_false_pos,
                                  FUN = sum)
head(annual_hiv_false_pos)
annual_hiv_false_pos
...
The following plots show the annual and monthly breakdown of positive HIV tests entered into the EIP Database. The number of positive tests within each rapid-based testing program is noted above in the table, but it is planned to include the breakdown of CTR vs. EHE vs. Lab testing positives within this breakdown to show the contributon of eahc testing stream within the total of positive tests.
...
Annual HIV positive tests for for all HIV tests entered into the EIP database is shown in the graph below.


```{r, echo = FALSE, warning = FALSE}

months_elapsed <- 3
current_annual_hiv_pos_2026 <- annual_hiv_pos$hiv_positive_test[annual_hiv_pos$Date == 2026]
projected_annual_hiv_pos_2026 <- (current_annual_hiv_pos_2026 / months_elapsed) * 12
hiv_pos_tests_projection <- data.frame(
  Date = c(2025, 2026),
  hiv_positive_test = c(
    annual_hiv_pos$hiv_positive_test[annual_hiv_pos$Date == 2025],
    projected_annual_hiv_pos_2026))



annual_hiv_pos_plot <- plot_ly(data = annual_hiv_pos,
                               x = ~Date,
                               y = ~hiv_positive_test,
                               type = 'scatter',
                               mode = 'lines+markers',
                               name = 'Total HIV Positive Tests',
                               marker = list(color = 'blue'),
                               text = ~hiv_positive_test,
                               textposition = 'auto') %>%
  add_trace(data = hiv_pos_tests_projection,
            x = ~Date,
            y = ~hiv_positive_test,
            type = 'scatter',
            mode = 'lines+markers',
            name = 'Projected 2026',
            line = list(dash = 'dash', color = 'lightgrey'),
            marker = list(color = 'lightgrey'),
            text = ~round(hiv_positive_test, 0),
            textposition = 'top center') %>%
  layout(xaxis = list(dtick = "M12", tickformat = "%Y"),
         yaxis = list(title = 'Number of Positive HIV Tests',
      range = c(0,
        max(
          c(annual_hiv_pos$hiv_positive_test,
            hiv_pos_tests_projection$hiv_positive_test),
          na.rm = TRUE) * 1.1)))

config(annual_hiv_pos_plot, displayModeBar = FALSE)
...
The figure below shows the monthly HIV positive testing numbers for all HIV tests entered into the EIP database.


```{r, echo = FALSE, warning = FALSE}

monthly_hiv_pos <- monthly_hiv_pos %>%
  mutate(Date = as.Date(paste0("01/", Date), format = "%d/%m/%Y")) %>%
  arrange(Date)

monthly_hiv_pos_plot <- plot_ly(data = monthly_hiv_pos,
                                x = ~Date,
                                y = ~hiv_positive_test,
                                type = 'scatter',
                                mode = 'lines+markers',
                                name = 'Positive HIV Tests',
                                text = ~hiv_positive_test, textposition = 'auto',
                                marker = list(color = 'darkblue')) %>%
  
  layout(xaxis = list(type = "date"), yaxis = list(title = 'Number of Positive HIV Tests'),
         barmode = 'overlay')

config(monthly_hiv_pos_plot, displayModeBar = FALSE)
...
According to the Centers for Disease Control and Prevention (CDC), antibody tests are about 99% specific, meaning they produce very few false positives. However, the specificity can sometimes be affected by various factors, which can lead to misleading test results. False positive tests are determined when an initial reactive test occurs but their confirmatory or follow-up lab testing indicates a negative result. Factors that may affect the specificity of the HIV test include the following:
...
- Additionally, recent vaccinations or pregnancy that may temporarily alter antibody and hormone levels

False positive tests can be captured in the EIP database when either rapid or lab tests are entered onto the HIV Testing form. Individual test results are entered, as well as the final interpretation of all individual tests conducted. If any of the individual tests are reactive/positive, but the final status is marked as negative (based on follow-up confirmatory testing), then the interpretation is counted as a false positive HIV test result. The overall number of false positive HIV tests within the EIP is [**`r hiv_false_positives`**]{style="color: red;"}. The annual and monthly breakdown of false positive tests are provided below.

::: {.panel-tabset}

### Annual

Annual HIV false positive tests for for all HIV tests entered into the EIP database is shown in the graph below.


```{r, echo = FALSE, warning = FALSE}

current_annual_hiv_false_pos_2026 <- annual_hiv_false_pos$hiv_false_positive_calc[annual_hiv_false_pos$Date == 2026]
projected_annual_hiv_false_pos_2026 <- (current_annual_hiv_false_pos_2026 / months_elapsed) * 12
hiv_false_pos_projection <- data.frame(
  Date = c(2025, 2026),
  hiv_false_positive_calc = c(
    annual_hiv_false_pos$hiv_false_positive_calc[annual_hiv_false_pos$Date == 2025],
    projected_annual_hiv_false_pos_2026))


annual_hiv_false_pos_plot <- plot_ly(data = annual_hiv_false_pos,
                                     x = ~Date,
                                     y = ~hiv_false_positive_calc,
                                     type = 'scatter',
                                     mode = 'lines+markers',
                                     name = 'Total False Positive Tests',
                                     marker = list(color = 'blue'),
                                     text = ~hiv_false_positive_calc,
                                     textposition = 'auto') %>%
  add_trace(
    data = hiv_false_pos_projection,
    x = ~Date,
    y = ~hiv_false_positive_calc,
    type = 'scatter',
    mode = 'lines+markers',
    ...
    line = list(dash = 'dash', color = 'lightgrey'),
    marker = list(color = 'lightgrey'),
    text = ~round(hiv_false_positive_calc, 0),
    textposition = 'top center') %>%
  
  layout(xaxis = list(dtick = "M12", tickformat = "%Y"),
         yaxis = list(title = 'Number of False Positive HIV Tests',
      range = c(0,
        max(
          c(annual_hiv_false_pos$hiv_false_positive_calc,
            hiv_false_pos_projection$hiv_false_positive_calc),
          na.rm = TRUE) * 1.1)),
         barmode = 'overlay')

config(annual_hiv_false_pos_plot, displayModeBar = FALSE)
...
The figure below shows the monthly HIV false positive testing numbers for all HIV tests entered into the EIP database.


```{r, echo = FALSE, warning = FALSE}

monthly_hiv_false_pos <- monthly_hiv_false_pos %>%
  mutate(Date = as.Date(paste0("01/", Date), format = "%d/%m/%Y"))

monthly_hiv_false_pos_plot <- plot_ly(data = monthly_hiv_false_pos,
                                    x = ~Date,
                                    y = ~hiv_false_positive_calc,
                                    type = 'bar',
                                    name = 'False Positive HIV Tests',
                                    text = ~hiv_false_positive_calc, textposition = 'auto',
                                    marker = list(color = 'lightskyblue')) %>%
  
  layout(yaxis = list(title = 'Number of False Positive HIV Tests'),
         barmode = 'overlay')

config(monthly_hiv_false_pos_plot, displayModeBar = FALSE)
